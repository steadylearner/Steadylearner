...
<!-- Image - Diesel Logo Image -->
<!-- Description - Rust Diesel Image from the official site -->

Before I divde deep to write a sitemap for [Steadylearner][Steadylearner], Which is built with React Frontend and Rust Backend, Building a sitemap for dynamic pages seemed to be solved problem. 

There were already some sitemap builders in React development environment and I tried to use them for the website. But it wans't that helpful for the website with dynamic page using databases, because they just extract path parts from react-router and write sitemap.xml or sitemap.txt.

So I thought that it would be better to find the solution at serverside using Rust because I already use Rust Diesel to handle database and what I should do will be just use datas to write sitemap.
...
In the next post, I will help you to write **sitemap.txt** file from the **sitemap.xml**. You should have read that some search engine uses it and you may not need to use some react packages to build sitemap.txt anymore if you are a React Developer and use it also.
